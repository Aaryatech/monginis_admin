...
<body>
	...
	<c:url var="getBillList" value="/getStationReportByRoute"></c:url>
	<div class="container" id="main-container">
		...
		<div id="main-content">
			...
			<div class="box">
				<div class="box-title">
					<h3>
						<i class="fa fa-bars"></i>Station Report
					</h3>
					...
					<div class="row">
						...
						<div class="form-group">
							...
							<div class="col-sm-3 col-lg-4">
								...
								<select data-placeholder="Choose Station"
									...
									<c:forEach items="${stationList}" var="station"
										varStatus="count">
										<option value="${station.stationId}"><c:out
												value="${station.stationNo}" /></option>
									</c:forEach>
								</select>
							</div>
																		...
																		function(
																			...
																			tr
																					.append($(
																							'<td></td>')
																							.html(
																									"<input type=text style='text-align:right; width:70px' class=form-control name=itemQty"+franchise.frId+""+item.id+" id=itemQty"+franchise.frId+""+item.id+" value=0 disabled/>"));
																		});
														tr
																.append($(
																		'<td></td>')
																		.html(
																				"<input type=text style='text-align:right; width:70px' class=form-control name=total"+key+""+item.id+" id=total"+key+""+item.id+" value=0 disabled/>"));
														$('#table_grid tbody')
																.append(tr);
													...
													function(key, report) {
														...
														document
																.getElementById('itemQty'
																		+ report.frId
																		+ ''
																		+ report.itemId).value = report.billQty;

													});

									$.each(data.itemList, function(key, item) {
										var total = 0;
										$.each(data.frList, function(key,
												franchise) {
											var tot = parseFloat($(
													"#itemQty" + franchise.frId
															+ '' + item.id)
													.val());
										...
										document.getElementById('total' + key
												+ '' + item.id).value = total;
									});
		...
		<script type="text/javascript">
			function genPdf() {
				...
				window.open('pdfForDisReport?url=pdf/getStationReportPdf/'
